...
export const AdminDashboard: React.FC = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b-2 border-ink/40 pb-4">
        <div>
          <h1 className="font-stencil text-3xl uppercase tracking-widest text-text-primary flex items-center">
            <Shield size={28} className="mr-3 text-stamp-red" /> Admin Command & Control
          </h1>
      ...
      <div className="flex flex-wrap bg-kraft border-2 border-ink/40 p-1 rounded-xs gap-1 font-mono text-xs font-bold uppercase">
        <button
        ...
        <button
          ...
          className={`px-3 py-2 rounded-xs flex items-center ${
            activeTab === 'zones' ? 'bg-ink text-paper shadow-sm' : 'text-text-primary hover:bg-paper/50'
          }`}
        >
        ...
        <button
          ...
          className={`px-3 py-2 rounded-xs flex items-center ${
            activeTab === 'rates' ? 'bg-ink text-paper shadow-sm' : 'text-text-primary hover:bg-paper/50'
          }`}
        >
        ...
        <button
          ...
          className={`px-3 py-2 rounded-xs flex items-center ${
            activeTab === 'cod' ? 'bg-ink text-paper shadow-sm' : 'text-text-primary hover:bg-paper/50'
          }`}
        >
        ...
        <button
          ...
          className={`px-3 py-2 rounded-xs flex items-center ${
            activeTab === 'agents' ? 'bg-ink text-paper shadow-sm' : 'text-text-primary hover:bg-paper/50'
          }`}
        >
        ...
        <button
          ...
          className={`px-3 py-2 rounded-xs flex items-center ${
            activeTab === 'notifications' ? 'bg-ink text-paper shadow-sm' : 'text-text-primary hover:bg-paper/50'
          }`}
        >
      ...
      {activeTab === 'orders' && (
        <div className="space-y-6">
          {/* Filters Bar */}
          <div className="bg-kraft/20 border-2 border-ink/40 p-4 rounded-xs flex flex-wrap items-center justify-between gap-4 font-mono text-xs">
            <div className="flex flex-wrap items-center gap-3">
              <span className="font-bold text-text-primary uppercase">Filters:</span>
              <select
                value={statusFilter}
                onChange={e => setStatusFilter(e.target.value)}
                className="bg-paper text-text-primary border border-ink/40 p-1.5 font-bold rounded-xs"
              >
            ...
            <button
              onClick={fetchOrders}
              className="bg-ink text-paper font-bold uppercase px-3 py-1.5 rounded-xs flex items-center"
            >
          ...
          {/* Orders Table */}
          <div className="bg-paper border-2 border-ink/40 rounded-xs overflow-x-auto shadow-md">
            <table className="w-full text-left border-collapse font-mono text-xs">
              <thead>
                <tr className="bg-ink text-paper uppercase border-b-2 border-ink">
              ...
              <tbody className="divide-y divide-ink/20">
                {orders.map(order => (
                  <tr key={order.id} className="hover:bg-kraft/30">
                    <td className="p-3 font-bold text-text-primary">
                      #{order.id.slice(0, 8).toUpperCase()}
                    </td>
                    <td className="p-3">
                      <div className="text-text-primary">{order.customer?.name}</div>
                      <div className="text-[10px] text-text-muted">{order.customer?.email}</div>
                    </td>
                    <td className="p-3">
                      <div className="text-text-primary">{order.pickupPincode} → {order.dropPincode}</div>
                      <div className="text-[10px] text-text-muted">{order.orderType} // {order.paymentType}</div>
                    </td>
                    <td className="p-3 font-bold text-stamp-green">
                      ₹{order.totalCharge}
                    </td>
                    <td className="p-3">
                    ...
                    <td className="p-3">
                      <select
                        ...
                        className="bg-paper text-text-primary border border-ink/40 p-1 text-[11px] font-bold rounded-xs"
                      >
                    ...
                    <td className="p-3 flex items-center space-x-2">
                      ...
                      <select
                        ...
                        className="bg-stamp-red text-paper p-1 text-[11px] font-bold uppercase rounded-xs"
                      >
                      ...
                      <button
                        ...
                        className="bg-ink text-paper p-1.5 px-2 rounded-xs font-bold text-[11px]"
                      >
                        View
                      </button>
                    </td>
          ...
          {selectedOrder && (
            <div className="fixed inset-0 z-50 bg-ink/80 backdrop-blur-xs flex items-center justify-center p-4">
              <div className="bg-paper border-2 border-ink rounded-xs max-w-2xl w-full p-6 shadow-2xl relative max-h-[90vh] overflow-y-auto">
                <button
                  onClick={() => setSelectedOrder(null)}
                  className="absolute right-3 top-3 font-mono font-bold text-xs bg-ink text-paper px-2 py-1"
                >
                  CLOSE [X]
                </button>
                <div className="font-stencil text-2xl text-text-primary mb-4">
      ...
      {activeTab === 'zones' && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          <div className="lg:col-span-6 space-y-6">
            <form onSubmit={handleCreateZone} className="bg-paper border-2 border-ink/40 p-6 rounded-xs shadow-md space-y-4 font-mono text-xs">
              <div className="font-stencil text-lg uppercase text-text-primary border-b border-ink/20 pb-2">
                Create New Logistics Zone
              </div>

              <div>
                <label className="block font-bold uppercase mb-1 text-text-primary">Zone Name</label>
                <input
                  type="text"
                  value={newZoneName}
                  onChange={e => setNewZoneName(e.target.value)}
                  placeholder="e.g. North Suburbs"
                  className="w-full bg-paper text-text-primary border border-ink/40 p-2 font-bold rounded-xs"
                  required
                />
              </div>

              <div>
                <label className="block font-bold uppercase mb-1 text-text-primary">Pincodes (Comma separated)</label>
                <input
                  type="text"
                ...
                />
                {zoneLookupLoading && (
                  <div className="text-[10px] text-text-muted font-mono flex items-center mt-1">
                    <RefreshCw size={10} className="animate-spin mr-1 text-stamp-amber" /> Resolving state locations...
                  </div>
                )}
                {!zoneLookupLoading && zoneLookupResults.length > 0 && (
                  <div className="mt-2 space-y-1.5 bg-kraft/30 p-2.5 rounded border border-ink/20 font-mono">
                    <div className="font-bold text-text-primary uppercase text-[9px]">State Lookup Resolution:</div>
                      ...
                      {zoneLookupResults.map(res => (
                        <span
                          ...
                          className={`px-1.5 py-0.5 rounded-xs border text-[10px] font-bold ${
                            res.isValid
                              ? 'bg-stamp-green/10 border-stamp-green text-stamp-green'
                              : 'bg-stamp-amber/20 border-stamp-amber text-stamp-amber'
                          }`}
                        >
                    ...
                    {(() => {
                      ...
                      if (validStates.length > 1) {
                        return (
                          <div className="mt-1.5 bg-stamp-red/10 border border-stamp-red text-stamp-red p-1.5 rounded-xs flex items-center font-bold text-[10px]">
              ...
              <button
                type="submit"
                className="w-full bg-ink text-paper font-bold uppercase py-2.5 rounded-xs hover:opacity-90"
              >
                Create Zone
              </button>
            </form>

            <form onSubmit={handleMapPincode} className="bg-paper border-2 border-ink/40 p-6 rounded-xs shadow-md space-y-4 font-mono text-xs">
              <div className="font-stencil text-lg uppercase text-text-primary border-b border-ink/20 pb-2">
                Map Pincode to Zone
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block font-bold uppercase mb-1 text-text-primary">Pincode</label>
                  <input
                    type="text"
                    value={mapPincode}
                  ...
                  {singleMapLoading && (
                    <div className="text-[10px] text-text-muted font-mono flex items-center mt-1">
                      <RefreshCw size={10} className="animate-spin mr-1 text-stamp-amber" /> Resolving state...
                    </div>
                  )}
                  {!singleMapLoading && singleMapLookup && (
                    <div className="mt-1 font-mono text-[10px]">
                      {singleMapLookup.isValid ? (
                        <span className="inline-block bg-stamp-green/10 border border-stamp-green text-stamp-green px-2 py-0.5 rounded-xs font-bold">
                          Detected: {singleMapLookup.state}{singleMapLookup.district ? ` (${singleMapLookup.district})` : ''}
                        </span>
                      ) : (
                        <span className="inline-block bg-stamp-amber/20 border border-stamp-amber text-stamp-amber px-2 py-0.5 rounded-xs font-bold">
                ...
                <div>
                  <label className="block font-bold uppercase mb-1 text-text-primary">Target Zone</label>
                  <select
                    value={mapZoneId}
              ...
              <button
                type="submit"
                className="w-full bg-stamp-green text-paper font-bold uppercase py-2.5 rounded-xs hover:opacity-90"
              >
                Map Pincode
              </button>
            </form>
          ...
          <div className="lg:col-span-6 space-y-4">
            <div className="font-stencil text-lg uppercase text-text-primary">Configured Zones Roster</div>
            <div className="space-y-3 font-mono text-xs">
              {zones.map(zone => (
                <div key={zone.id} className="bg-paper border-2 border-ink/40 p-4 rounded-xs shadow-sm">
                  <div className="font-bold text-base text-text-primary mb-1">{zone.name}</div>
                  <div className="text-text-secondary mb-2">Zone ID: {zone.id}</div>
                  <div className="flex flex-wrap gap-1">
                    <span className="font-bold text-[10px] text-text-muted uppercase">Mapped PINs:</span>
                    {zone.pincodeMaps?.map((pm: any) => (
                      <span key={pm.pincode} className="bg-kraft text-text-primary border border-ink/30 px-1.5 py-0.5 text-[10px] font-bold rounded-xs">
                        {pm.pincode}
                      </span>
                    ))}
                  </div>
      ...
      {activeTab === 'rates' && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 font-mono text-xs">
          <div className="lg:col-span-5">
            <form onSubmit={handleSaveRateCard} className="bg-paper border-2 border-ink/40 p-6 rounded-xs shadow-md space-y-4">
              <div className="font-stencil text-lg uppercase text-text-primary border-b border-ink/20 pb-2">
                Create / Update Rate Card
              </div>

              <div>
                <label className="block font-bold uppercase mb-1 text-text-primary">From Origin Zone</label>
                <select
                  value={rcFromZoneId}
              ...
              <div>
                <label className="block font-bold uppercase mb-1 text-text-primary">To Destination Zone</label>
                <select
                  value={rcToZoneId}
              ...
              <div>
                <label className="block font-bold uppercase mb-1 text-text-primary">Order Category</label>
                <div className="flex space-x-2">
                  <button
                    type="button"
                    onClick={() => setRcOrderType('B2C')}
                    className={`flex-1 py-2 border border-ink/40 rounded-xs font-bold ${
                      rcOrderType === 'B2C' ? 'bg-ink text-paper' : 'bg-paper text-text-primary'
                    }`}
                  >
                    B2C
                  </button>
                  <button
                    ...
                    className={`flex-1 py-2 border border-ink/40 rounded-xs font-bold ${
                      rcOrderType === 'B2B' ? 'bg-ink text-paper' : 'bg-paper text-text-primary'
                    }`}
                  >
                    B2B
                  </button>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block font-bold uppercase mb-1 text-text-primary">Base Rate (₹)</label>
                  <input
                    type="number"
                    step="0.5"
                    value={rcBaseRate}
                ...
                <div>
                  <label className="block font-bold uppercase mb-1 text-text-primary">Per Kg Rate (₹)</label>
                  <input
                    type="number"
                    step="0.5"
                    value={rcPerKgRate}
          ...
          <div className="lg:col-span-7 space-y-4">
            <div className="font-stencil text-lg uppercase text-text-primary">Configured Rate Matrix</div>
            <div className="bg-paper border-2 border-ink/40 rounded-xs overflow-x-auto shadow-md">
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-ink/20">
                  {rateCards.map(rc => (
                    <tr key={rc.id} className="hover:bg-kraft/30">
                      <td className="p-3 font-bold text-text-primary">
                        {rc.fromZone?.name} → {rc.toZone?.name}
                      </td>
                      <td className="p-3 uppercase font-bold text-text-secondary">{rc.orderType}</td>
                      <td className="p-3 font-bold text-text-primary">₹{Number(rc.baseRate).toFixed(2)}</td>
                      <td className="p-3 font-bold text-stamp-green">₹{Number(rc.perKgRate).toFixed(2)}</td>
                      <td className="p-3 font-bold text-text-primary">{rc.isIntraZone ? 'YES' : 'NO'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      {activeTab === 'cod' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 font-mono text-xs">
          <div className="bg-paper border-2 border-ink/40 p-6 rounded-xs shadow-md space-y-4">
            <div className="font-stencil text-lg uppercase text-text-primary border-b border-ink/20 pb-2">
              B2C Retail COD Config
            </div>

            <div>
              <label className="block font-bold uppercase mb-1 text-text-primary">Flat Fee (₹)</label>
              <input
                type="number"
                step="0.5"
                value={b2cFlat}
            ...
            <div>
              <label className="block font-bold uppercase mb-1 text-text-primary">Percentage Of Bill (%)</label>
              <input
                type="number"
                step="0.1"
                value={b2cPercent}
      ...
      {activeTab === 'agents' && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 font-mono text-xs">
          <div className="lg:col-span-5">
            <form onSubmit={handleCreateAgent} className="bg-paper border-2 border-ink/40 p-6 rounded-xs shadow-md space-y-4">
              <div className="font-stencil text-lg uppercase text-text-primary border-b border-ink/20 pb-2 flex items-center">
                <UserPlus size={18} className="mr-2" /> Register Delivery Agent
              </div>

              <div>
                <label className="block font-bold uppercase mb-1 text-text-primary">Agent Name</label>
                <input
                  type="text"
                  value={newAgentName}
              ...
              <div>
                <label className="block font-bold uppercase mb-1 text-text-primary">Email</label>
                <input
                  type="email"
                  value={newAgentEmail}
              ...
              <div>
                <label className="block font-bold uppercase mb-1 text-text-primary">Initial Password</label>
                <input
                  type="password"
                  value={newAgentPass}
              ...
              <div>
                <label className="block font-bold uppercase mb-1 text-text-primary">Assigned Base Zone</label>
                <select
          ...
          <div className="lg:col-span-7 space-y-4">
            <div className="font-stencil text-lg uppercase text-text-primary">Field Agent Radar Roster</div>
            <div className="space-y-3">
              {agents.map(agent => {
                const isAvailable = agent.availability === 'AVAILABLE';
                return (
                  <div key={agent.id} className="bg-paper border-2 border-ink/40 p-4 rounded-xs shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <div>
                      <div className="flex items-center space-x-2">
                        {/* Radar Pulse Indicator */}
                        <div className="relative flex items-center justify-center w-4 h-4">
                          <div className={`w-3 h-3 rounded-full ${isAvailable ? 'bg-stamp-green' : 'bg-stamp-red'}`} />
                          {isAvailable && (
                            <div className="absolute w-6 h-6 rounded-full bg-stamp-green/40 animate-radar-pulse" />
                          )}
                        </div>
                        <span className="font-bold text-base text-text-primary">{agent.user?.name}</span>
                      </div>
                    ...
                    <div className="flex items-center space-x-2 self-end sm:self-center">
                      <span className={`px-2.5 py-1 text-xs font-bold uppercase rounded-xs border border-ink/40 ${
                        isAvailable ? 'bg-stamp-green text-paper' : 'bg-stamp-red text-paper'
                      }`}>
                        {agent.availability}
                      </span>
                      ...
                      <button
                        ...
                        className="bg-kraft border border-ink/40 p-1.5 rounded-xs text-text-primary hover:opacity-90 transition-colors"
                      >
                        <Edit2 size={15} />
                      </button>
                      ...
                      <button
                        ...
                        className="bg-stamp-red/10 border border-stamp-red p-1.5 rounded-xs hover:bg-stamp-red hover:text-paper text-stamp-red transition-colors"
                      >
                        <Trash2 size={15} />
                      </button>
                    </div>
      ...
      {/* EDIT AGENT MODAL */}
      {editingAgent && (
        <div className="fixed inset-0 bg-ink/80 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <div className="bg-paper border-2 border-ink max-w-md w-full p-6 rounded-xs shadow-2xl space-y-4 font-mono text-xs relative">
            <button
            ...
            <div className="font-stencil text-xl uppercase text-text-primary border-b border-ink/30 pb-2 flex items-center">
              <Edit2 size={18} className="mr-2 text-stamp-amber" /> Modify Agent: {editingAgent.user?.name}
            </div>
            ...
            <form onSubmit={handleSaveEditAgent} className="space-y-3">
              ...
              <div>
                <label className="block font-bold uppercase mb-1 text-text-primary">Phone Number</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block font-bold uppercase mb-1 text-text-primary">Availability Status</label>
                <select
              ...
              <div>
                <label className="block font-bold uppercase mb-1 text-text-primary">Reset Password (Optional)</label>
                <input
                  type="password"
                  value={editAgentPass}
              ...
              <div className="pt-2 flex justify-end space-x-2">
                <button
                  type="button"
                  onClick={() => setEditingAgent(null)}
                  className="px-4 py-2 border border-ink/40 rounded-xs font-bold uppercase bg-kraft text-text-primary hover:opacity-90"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  className="px-4 py-2 border border-ink/40 rounded-xs font-bold uppercase bg-ink text-paper hover:opacity-90"
                >
                  Save Modifications
                </button>
              </div>
      ...
      {deletingAgent && (
        <div className="fixed inset-0 bg-ink/80 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <div className="bg-paper border-2 border-ink max-w-md w-full p-6 rounded-xs shadow-2xl space-y-4 font-mono text-xs relative">
            ...
            <div className="font-stencil text-xl uppercase text-stamp-red border-b border-ink/30 pb-2 flex items-center">
              <AlertTriangle size={20} className="mr-2 text-stamp-red" /> Confirm Agent Removal
            </div>

            <p className="text-text-primary leading-relaxed">
              Are you sure you want to remove agent <strong className="text-text-primary">{deletingAgent.user?.name}</strong> ({deletingAgent.user?.email}) from active duty in <strong className="text-text-primary">{deletingAgent.zone?.name}</strong>?
            </p>

            <div className="bg-stamp-amber/20 border border-stamp-amber p-2.5 rounded-xs text-[11px] text-stamp-amber font-bold">
            ...
            <div className="flex justify-end space-x-2 pt-2">
              ...
              <button
                ...
                className="px-4 py-2 border border-ink/40 rounded-xs font-bold uppercase bg-stamp-red text-paper hover:opacity-90"
              >
                Remove Agent
              </button>
            </div>
      ...
      {activeTab === 'notifications' && (
        <div className="space-y-4 font-mono text-xs">
          <div className="font-stencil text-lg uppercase text-text-primary">System Notification Audit Trail</div>
          <div className="bg-paper border-2 border-ink/40 rounded-xs overflow-x-auto shadow-md">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-ink text-paper uppercase border-b-2 border-ink">
              ...
              <tbody className="divide-y divide-ink/20">
                {notificationLogs.map(log => (
                  <tr key={log.id} className="hover:bg-kraft/30">
                    <td className="p-3 text-text-secondary">{new Date(log.createdAt).toLocaleString()}</td>
                    <td className="p-3 uppercase font-bold text-text-primary">{log.channel}</td>
                    <td className="p-3 font-bold text-text-primary">{log.toAddress}</td>
                    <td className="p-3 text-text-primary">{log.subject}</td>
                    <td className="p-3">
                      <span className={`px-2 py-0.5 rounded-xs font-bold text-[10px] ${
                        log.status === 'SENT' ? 'bg-stamp-green text-paper' : 'bg-stamp-red text-paper'
                      }`}>
                        {log.status}
                      </span>
                    </td>
